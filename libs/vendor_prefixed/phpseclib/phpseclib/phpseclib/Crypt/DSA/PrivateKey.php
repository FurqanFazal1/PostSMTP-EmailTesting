 ...
 */
namespace PostSMTPMailTester\Vendor\phpseclib3\Crypt\DSA;

use PostSMTPMailTester\Vendor\phpseclib3\Crypt\Common;
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\DSA;
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\DSA\Formats\Signature\ASN1 as ASN1Signature;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class PrivateKey extends \PostSMTPMailTester\Vendor\phpseclib3\Crypt\DSA implements \PostSMTPMailTester\Vendor\phpseclib3\Crypt\Common\PrivateKey
{
    ...
    public function getPublicKey()
    {
        ...
        return \PostSMTPMailTester\Vendor\phpseclib3\Crypt\DSA::loadFormat('PKCS8', $key)->withHash($this->hash->getHash())->withSignatureFormat($this->shortFormat);
    }
    /**
     * Create a signature
     *
    ...
    public function sign($message)
    {
        ...
        if (self::$engines['OpenSSL'] && \in_array($this->hash->getHash(), \openssl_get_md_methods())) {
            ...
            if ($result) {
                ...
                \extract(\PostSMTPMailTester\Vendor\phpseclib3\Crypt\DSA\Formats\Signature\ASN1::load($signature));
        ...
        $h = $this->bits2int($h);
        while (\true) {
            $k = \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger::randomRange(self::$one, $this->q->subtract(self::$one));
