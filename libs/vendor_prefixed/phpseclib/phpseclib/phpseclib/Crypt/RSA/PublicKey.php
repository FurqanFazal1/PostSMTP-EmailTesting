 ...
 */
namespace PostSMTPMailTester\Vendor\phpseclib3\Crypt\RSA;
...
use PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo;
 ...
 * @author  Jim Wigginton <terrafrost@php.net>
 */
class PublicKey extends \PostSMTPMailTester\Vendor\phpseclib3\Crypt\RSA implements \PostSMTPMailTester\Vendor\phpseclib3\Crypt\Common\PublicKey
{
     ...
     */
    private function exponentiate(\PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger $x)
    {
    ...
    private function rsassa_pkcs1_v1_5_verify($m, $s)
    {
        ...
        try {
            ...
            $exception = \true;
        } catch (\PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedAlgorithmException $e) {
            $r2 = \false;
        }
        if ($exception) {
    ...
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        if (\PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::shift($em, 2) != "\0\1") {
            return \false;
        }
        $em = \ltrim($em, "�");
        if (\PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::shift($em) != "\0") {
            return \false;
        }
        $decoded = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::decodeBER($em);
        ...
        if (!isset($oids)) {
            ...
            \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::loadOIDs($oids);
        }
        $decoded = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo::MAP);
        ...
        $hash = new \PostSMTPMailTester\Vendor\phpseclib3\Crypt\Hash($hash);
        $em = $hash->hash($m);
        $em2 = $decoded['digest'];
    ...
    private function rsaes_pkcs1_v1_5_encrypt($m, $pkcs15_compat = \false)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \PostSMTPMailTester\Vendor\phpseclib3\Crypt\Random::string($psLen - \strlen($ps));
    ...
    private function rsaes_oaep_encrypt($m)
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \PostSMTPMailTester\Vendor\phpseclib3\Crypt\Random::string($this->hLen);
    ...
    public function toString($type, array $options = [])
    {
        ...
        if ($type == \PostSMTPMailTester\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
            if ($this->signaturePadding == self::SIGNATURE_PSS) {
                ...
                throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedFormatException('The PSS format can only be used when the signature method has been explicitly set to PSS');
            }
        }
    ...
    {
        $new = new \PostSMTPMailTester\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
