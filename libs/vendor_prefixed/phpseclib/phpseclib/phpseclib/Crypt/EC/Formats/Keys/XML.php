...
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery as MontgomeryCurve;
...
use PostSMTPMailTester\Vendor\phpseclib3\Exception\BadConfigurationException;
...
abstract class XML
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (!\class_exists('DOMDocument')) {
            throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\BadConfigurationException('The dom extension is not setup correctly on this system');
        }
    ...
    {
        return \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_decode(\str_replace(["\r", "\n", ' ', "\t"], '', $value));
    }
    /**
     ...
     * @return object[]
     */
    private static function extractPointRFC4050(\DOMXPath $xpath, \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
        ...
        $point = [$curve->convertInteger(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($x->item(0)->getAttribute('Value'))), $curve->convertInteger(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($y->item(0)->getAttribute('Value')))];
    ...
    private static function loadCurveByParam(\DOMXPath $xpath)
    {
        ...
        if ($namedCurve->length == 1) {
            ...
            if ($name === \false) {
                throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Curve with OID of ' . $oid . ' is not supported');
            }
            $curve = '\\PostSMTPMailTester\\Vendor\\phpseclib3\\Crypt\\EC\\Curves\\' . $name;
            if (!\class_exists($curve)) {
                throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $name . ' is not supported');
            }
            return new $curve();
        }
        ...
        switch ($type) {
            case 'prime-field':
                ...
                $curve->setModulo(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($p, 256));
                $curve->setCoefficients(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($a, 256), new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($b, 256));
                $point = self::extractPoint("\0" . $base, $curve);
                $curve->setBasePoint(...$point);
                $curve->setOrder(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($order, 256));
                return $curve;
            case 'gnb':
            case 'tnb':
            case 'pnb':
            default:
                throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $type . ' is not supported');
        }
    }
    /**
     * Returns an instance of \phpseclib3\Crypt\EC\BaseCurves\Base based
    ...
    private static function loadCurveByParamRFC4050(\DOMXPath $xpath)
    {
        ...
        switch ($type) {
            case 'prime-field':
                $curve = new \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime();
                $p = \str_replace(["\r", "\n", ' ', "\t"], '', $p);
                $curve->setModulo(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($p));
                $a = \str_replace(["\r", "\n", ' ', "\t"], '', $a);
                $b = \str_replace(["\r", "\n", ' ', "\t"], '', $b);
                $curve->setCoefficients(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($a), new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($b));
                $x = \str_replace(["\r", "\n", ' ', "\t"], '', $x);
                $y = \str_replace(["\r", "\n", ' ', "\t"], '', $y);
                $curve->setBasePoint(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($x), new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($y));
                $order = \str_replace(["\r", "\n", ' ', "\t"], '', $order);
                $curve->setOrder(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($order));
                return $curve;
            default:
                throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $type . ' is not supported');
        }
     ...
     * @return string
     */
    public static function savePublicKey(\PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, array $options = [])
    {
        self::initialize_static_variables();
        if ($curve instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards || $curve instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('TwistedEdwards and Montgomery Curves are not supported');
        }
        ...
        return '<' . $pre . 'ECDSAKeyValue xmlns' . $post . '="http://www.w3.org/2009/xmldsig11#">' . "\r\n" . self::encodeXMLParameters($curve, $pre, $options) . "\r\n" . '<' . $pre . 'PublicKey>' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($publicKey) . '</' . $pre . 'PublicKey>' . "\r\n" . '</' . $pre . 'ECDSAKeyValue>';
    }
    /**
     * Encode Parameters
     *
     ...
     * @return string|false
     */
    private static function encodeXMLParameters(\PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, $pre, array $options = [])
    {
        ...
        if (self::$rfc4050) {
            ...
            switch ($temp['fieldID']['fieldType']) {
                case 'prime-field':
                    ...
                    break;
                default:
                    throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $temp['fieldID']['fieldType'] . ' is not supported');
            }
        ...
        if (isset($result['specifiedCurve'])) {
            ...
            switch ($temp['fieldID']['fieldType']) {
                case 'prime-field':
                    $xml .= '<' . $pre . 'Prime>' . "\r\n" . '<' . $pre . 'P>' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($temp['fieldID']['parameters']->toBytes()) . '</' . $pre . 'P>' . "\r\n" . '</' . $pre . 'Prime>' . "\r\n";
                    break;
                default:
                    throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $temp['fieldID']['fieldType'] . ' is not supported');
            }
            $xml .= '</' . $pre . 'FieldID>' . "\r\n" . '<' . $pre . 'Curve>' . "\r\n" . '<' . $pre . 'A>' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($temp['curve']['a']) . '</' . $pre . 'A>' . "\r\n" . '<' . $pre . 'B>' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($temp['curve']['b']) . '</' . $pre . 'B>' . "\r\n" . '</' . $pre . 'Curve>' . "\r\n" . '<' . $pre . 'Base>' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($temp['base']) . '</' . $pre . 'Base>' . "\r\n" . '<' . $pre . 'Order>' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::base64_encode($temp['order']) . '</' . $pre . 'Order>' . "\r\n" . '</' . $pre . 'ECParameters>';
            return $xml;
        }
    }
}
