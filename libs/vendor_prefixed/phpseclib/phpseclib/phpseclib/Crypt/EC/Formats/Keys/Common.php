 ...
 */
namespace PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\Formats\Keys;

use PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings;
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base as BaseCurve;
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary as BinaryCurve;
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime as PrimeCurve;
use PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards as TwistedEdwardsCurve;
use PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException;
...
trait Common
{
     ...
     */
    public static function setImplicitCurve(\PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
    ...
    protected static function loadCurveByParam(array $params)
    {
        ...
        if (isset($params['namedCurve'])) {
            $curve = '\\PostSMTPMailTester\\Vendor\\phpseclib3\\Crypt\\EC\\Curves\\' . $params['namedCurve'];
            if (!\class_exists($curve)) {
                throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Named Curve of ' . $params['namedCurve'] . ' is not supported');
            }
            return new $curve();
        }
        ...
        if (isset($params['specifiedCurve'])) {
            ...
            switch ($data['fieldID']['fieldType']) {
                case 'prime-field':
                    $curve = new \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime();
                    $curve->setModulo($data['fieldID']['parameters']);
                    $curve->setCoefficients(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($data['curve']['a'], 256), new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($data['curve']['b'], 256));
                    $point = self::extractPoint("\0" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
                case 'characteristic-two-field':
                    $curve = new \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary();
                    $params = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::decodeBER($data['fieldID']['parameters']);
                    $params = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::asn1map($params[0], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\Characteristic_two::MAP);
                    ...
                    switch ($params['basis']) {
                        case 'tpBasis':
                            ...
                            break;
                        case 'ppBasis':
                            $temp = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::decodeBER($params['parameters']);
                            $temp = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::asn1map($temp[0], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\Pentanomial::MAP);
                    ...
                    $len = \ceil($modulo[0] / 8);
                    $curve->setCoefficients(\PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($data['curve']['a']), \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($data['curve']['b']));
                    $point = self::extractPoint("\0" . $data['base'], $curve);
                    $curve->setBasePoint(...$point);
                    $curve->setOrder($data['order']);
                    return $curve;
                default:
                    throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Field Type of ' . $data['fieldID']['fieldType'] . ' is not supported');
            }
        }
     ...
     * @return object[]
     */
    public static function extractPoint($str, \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve)
    {
        if ($curve instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            ...
            $y[0] = $y[0] & \chr(0x7f);
            $y = new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($y, 256);
        ...
        if (($val = \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::shift($str)) != "\0") {
            throw new \UnexpectedValueException('extractPoint expects the first byte to be null - not ' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($val));
        }
        if ($str == "\0") {
            return [];
        }
        ...
        if ($keylen == 2 * $order + 1) {
            ...
            list(, $w, $x, $y) = $matches;
            if ($w != "\4") {
                throw new \UnexpectedValueException('The first byte of an uncompressed point should be 04 - not ' . \PostSMTPMailTester\Vendor\phpseclib3\Common\Functions\Strings::bin2hex($val));
            }
            $point = [$curve->convertInteger(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($x, 256)), $curve->convertInteger(new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($y, 256))];
            if (!$curve->verifyPoint($point)) {
                throw new \RuntimeException('Unable to verify that point exists on curve');
            }
            return $point;
     ...
     * @return string|false
     */
    private static function encodeParameters(\PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Base $curve, $returnArray = \false, array $options = [])
    {
        ...
        if ($useNamedCurves) {
            if (isset(self::$curveOIDs[$name])) {
                ...
                return $returnArray ? ['namedCurve' => $name] : \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::encodeDER(['namedCurve' => $name], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
            }
            foreach (new \DirectoryIterator(__DIR__ . '/../../Curves/') as $file) {
                ...
                $class = 'PostSMTPMailTester\\Vendor\\phpseclib3\\Crypt\\EC\\Curves\\' . $testName;
                ...
                switch ($name) {
                    case 'Prime':
                        if (!$candidate instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime) {
                            break;
                        }
                        ...
                        return $returnArray ? ['namedCurve' => $testName] : \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::encodeDER(['namedCurve' => $testName], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
                    case 'Binary':
                        if (!$candidate instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary) {
                            break;
                        }
        ...
        $x = $point[0]->toBytes();
        $y = $point[1]->toBytes();
        if ($curve instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Prime) {
            /*
             * valid versions are:
             *
             * ecdpVer1:
            ...
            return $returnArray ? ['specifiedCurve' => $data] : \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::encodeDER(['specifiedCurve' => $data], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        }
        if ($curve instanceof \PostSMTPMailTester\Vendor\phpseclib3\Crypt\EC\BaseCurves\Binary) {
            ...
            switch ($basis) {
                case 3:
                    $basis = 'tpBasis';
                    $modulo = new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($modulo[0]);
                    break;
                case 5:
                    $basis = 'ppBasis';
                    // these should be in strictly ascending order (hence the commented out rsort above)
                    $modulo = ['k1' => new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($modulo[2]), 'k2' => new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($modulo[1]), 'k3' => new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($modulo[0])];
                    $modulo = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::encodeDER($modulo, \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\Pentanomial::MAP);
                    $modulo = new \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Element($modulo);
            }
            $params = \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1::encodeDER(['m' => new \PostSMTPMailTester\Vendor\phpseclib3\Math\BigInteger($m), 'basis' => $basis, 'parameters' => $modulo], \PostSMTPMailTester\Vendor\phpseclib3\File\ASN1\Maps\Characteristic_two::MAP);
        ...
        throw new \PostSMTPMailTester\Vendor\phpseclib3\Exception\UnsupportedCurveException('Curve cannot be serialized');
    }
    /**
     * Use Specified Curve
     *
