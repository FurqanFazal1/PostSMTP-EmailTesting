...
trait ServiceAccountSignerTrait
{
    ...
    public function signBlob($stringToSign, $forceOpenssl = \false)
    {
        ...
        $signedString = '';
        if (\class_exists('PostSMTPMailTester\\Vendor\\phpseclib\\Crypt\\RSA') && !$forceOpenssl) {
            $rsa = new \PostSMTPMailTester\Vendor\phpseclib\Crypt\RSA();
            $rsa->loadKey($privateKey);
            $rsa->setSignatureMode(\PostSMTPMailTester\Vendor\phpseclib\Crypt\RSA::SIGNATURE_PKCS1);
            $rsa->setHash('sha256');
