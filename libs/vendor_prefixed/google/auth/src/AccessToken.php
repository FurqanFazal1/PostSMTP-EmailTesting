...
use DateTime;
use Exception;
use PostSMTPMailTester\Vendor\Firebase\JWT\ExpiredException;
use PostSMTPMailTester\Vendor\Firebase\JWT\JWT;
use PostSMTPMailTester\Vendor\Firebase\JWT\SignatureInvalidException;
...
use PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Request;
use PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use PostSMTPMailTester\Vendor\phpseclib\Crypt\RSA;
use PostSMTPMailTester\Vendor\phpseclib\Math\BigInteger;
use PostSMTPMailTester\Vendor\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use PostSMTPMailTester\Vendor\SimpleJWT\InvalidTokenException;
use PostSMTPMailTester\Vendor\SimpleJWT\JWT as SimpleJWT;
use PostSMTPMailTester\Vendor\SimpleJWT\Keys\KeyFactory;
use PostSMTPMailTester\Vendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \PostSMTPMailTester\Vendor\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \PostSMTPMailTester\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build(\PostSMTPMailTester\Vendor\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \PostSMTPMailTester\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        } catch (\PostSMTPMailTester\Vendor\Firebase\JWT\ExpiredException $e) {
            // firebase/php-jwt 3+
        } catch (\PostSMTPMailTester\Vendor\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (\PostSMTPMailTester\Vendor\Firebase\JWT\SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\PostSMTPMailTester\Vendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
        } catch (\PostSMTPMailTester\Vendor\SimpleJWT\InvalidTokenException $e) {
            // simplejwt
        } catch (\PostSMTPMailTester\Vendor\Google\Auth\DomainException $e) {
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \PostSMTPMailTester\Vendor\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\PostSMTPMailTester\Vendor\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa = new \PostSMTPMailTester\Vendor\phpseclib\Crypt\RSA();
            $rsa->loadKey(['n' => new \PostSMTPMailTester\Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \PostSMTPMailTester\Vendor\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Utils::streamFor(\http_build_query(['token' => $token]));
        $request = new \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    private function checkAndInitializePhpsec()
    {
        ...
        if (!\class_exists('PostSMTPMailTester\\Vendor\\phpseclib\\Crypt\\RSA')) {
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('PostSMTPMailTester\\Vendor\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            if (!\defined('PostSMTPMailTester\\Vendor\\MATH_BIGINTEGER_OPENSSL_ENABLED')) {
                \define('PostSMTPMailTester\\Vendor\\MATH_BIGINTEGER_OPENSSL_ENABLED', \true);
            }
            if (!\defined('PostSMTPMailTester\\Vendor\\CRYPT_RSA_MODE')) {
                \define('PostSMTPMailTester\\Vendor\\CRYPT_RSA_MODE', \PostSMTPMailTester\Vendor\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = 'PostSMTPMailTester\\Vendor\\Firebase\\JWT\\JWT';
