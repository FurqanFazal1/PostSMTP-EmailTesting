...
use PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Query;
 ...
 */
class OAuth2 implements \PostSMTPMailTester\Vendor\Google\Auth\FetchAuthTokenInterface
{
    ...
    public function generateCredentialsRequest()
    {
        ...
        return new \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Request('POST', $uri, $headers, \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Query::build($params));
    }
    /**
     ...
     * @throws \Exception
     */
    public function parseTokenResponse(\PostSMTPMailTester\Vendor\Psr\Http\Message\ResponseInterface $resp)
    {
    ...
    public function buildFullAuthorizationUri(array $config = [])
    {
        ...
        $existingParams = \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Query::parse($result->getQuery());
        $result = $result->withQuery(\PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Query::build(\array_merge($existingParams, $params)));
    ...
    {
        return \PostSMTPMailTester\Vendor\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        return \PostSMTPMailTester\Vendor\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
