...
use PostSMTPMailTester\Vendor\Google\Auth\Credentials\InsecureCredentials;
 ...
 */
abstract class CredentialsLoader implements \PostSMTPMailTester\Vendor\Google\Auth\FetchAuthTokenInterface, \PostSMTPMailTester\Vendor\Google\Auth\UpdateMetadataInterface
{
    ...
    private static function getGuzzleMajorVersion()
    {
        if (\defined('PostSMTPMailTester\\Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            return \PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        }
        if (\defined('PostSMTPMailTester\\Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            return (int) \substr(\PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
    ...
    public static function makeCredentials($scope, array $jsonKey, $defaultScope = null)
    {
        ...
        if ($jsonKey['type'] == 'service_account') {
            ...
            return new \PostSMTPMailTester\Vendor\Google\Auth\Credentials\ServiceAccountCredentials($scope, $jsonKey);
        }
        if ($jsonKey['type'] == 'authorized_user') {
            $anyScope = $scope ?: $defaultScope;
            return new \PostSMTPMailTester\Vendor\Google\Auth\Credentials\UserRefreshCredentials($anyScope, $jsonKey);
        }
     ...
     */
    public static function makeHttpClient(\PostSMTPMailTester\Vendor\Google\Auth\FetchAuthTokenInterface $fetcher, array $httpClientOptions = [], callable $httpHandler = null, callable $tokenCallback = null)
    {
        if (self::getGuzzleMajorVersion() === 5) {
            $client = new \PostSMTPMailTester\Vendor\GuzzleHttp\Client($httpClientOptions);
            $client->setDefaultOption('auth', 'google_auth');
            $subscriber = new \PostSMTPMailTester\Vendor\Google\Auth\Subscriber\AuthTokenSubscriber($fetcher, $httpHandler, $tokenCallback);
            $client->getEmitter()->attach($subscriber);
            return $client;
        }
        $middleware = new \PostSMTPMailTester\Vendor\Google\Auth\Middleware\AuthTokenMiddleware($fetcher, $httpHandler, $tokenCallback);
        $stack = \PostSMTPMailTester\Vendor\GuzzleHttp\HandlerStack::create();
        $stack->push($middleware);
        return new \PostSMTPMailTester\Vendor\GuzzleHttp\Client(['handler' => $stack, 'auth' => 'google_auth'] + $httpClientOptions);
    }
    /**
    ...
    {
        return new \PostSMTPMailTester\Vendor\Google\Auth\Credentials\InsecureCredentials();
    }
    /**
