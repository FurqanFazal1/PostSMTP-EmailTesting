...
class SimpleMiddleware
{
    ...
    public function __invoke(callable $handler)
    {
        return function (\PostSMTPMailTester\Vendor\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            ...
            $query = \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Query::parse($request->getUri()->getQuery());
            $params = \array_merge($query, $this->config);
            $uri = $request->getUri()->withQuery(\PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Query::build($params));
