...
class Batch
{
    ...
    private $rootUrl;
    private $batchPath;
    public function __construct(\PostSMTPMailTester\Vendor\Google\Client $client, $boundary = \false, $rootUrl = null, $batchPath = null)
    {
        $this->client = $client;
    ...
    public function add(\PostSMTPMailTester\Vendor\Psr\Http\Message\RequestInterface $request, $key = \false)
    {
        if (\false == $key) {
            $key = \mt_rand();
        }
...
EOF;
        ...
        $request = new \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Request('POST', $url, $headers, $body);
        $response = $this->client->execute($request);
        return $this->parseResponse($response, $classes);
    }
    public function parseResponse(\PostSMTPMailTester\Vendor\Psr\Http\Message\ResponseInterface $response, $classes = array())
    {
        ...
        if (!empty($body)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Response($status, $partHeaders, \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Utils::streamFor($partBody));
                    // Need content id.
                    $key = $headers['content-id'];
                    try {
                        $response = \PostSMTPMailTester\Vendor\Google\Http\REST::decodeHttpResponse($response, $requests[$i - 1]);
                    } catch (\PostSMTPMailTester\Vendor\Google\Service\Exception $e) {
