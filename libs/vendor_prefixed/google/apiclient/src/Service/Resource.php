 ...
 */
namespace PostSMTPMailTester\Vendor\Google\Service;

use PostSMTPMailTester\Vendor\Google\Model;
use PostSMTPMailTester\Vendor\Google\Http\MediaFileUpload;
use PostSMTPMailTester\Vendor\Google\Exception as GoogleException;
use PostSMTPMailTester\Vendor\Google\Utils\UriTemplate;
...
class Resource
{
    ...
    public function call($name, $arguments, $expectedClass = null)
    {
        if (!isset($this->methods[$name])) {
            ...
            throw new \PostSMTPMailTester\Vendor\Google\Exception("Unknown function: " . "{$this->serviceName}->{$this->resourceName}->{$name}()");
        }
        ...
        if (isset($parameters['postBody'])) {
            if ($parameters['postBody'] instanceof \PostSMTPMailTester\Vendor\Google\Model) {
        ...
        foreach ($parameters as $key => $val) {
            if ($key != 'postBody' && !isset($method['parameters'][$key])) {
                ...
                throw new \PostSMTPMailTester\Vendor\Google\Exception("({$name}) unknown parameter: '{$key}'");
            }
        }
        foreach ($method['parameters'] as $paramName => $paramSpec) {
            if (isset($paramSpec['required']) && $paramSpec['required'] && !isset($parameters[$paramName])) {
                ...
                throw new \PostSMTPMailTester\Vendor\Google\Exception("({$name}) missing required param: '{$paramName}'");
            }
        ...
        $request = new \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Request($method['httpMethod'], $url, ['content-type' => 'application/json'], $postBody ? \json_encode($postBody) : '');
        // support uploads
        if (isset($parameters['data'])) {
            $mimeType = isset($parameters['mimeType']) ? $parameters['mimeType']['value'] : 'application/octet-stream';
            $data = $parameters['data']['value'];
            $upload = new \PostSMTPMailTester\Vendor\Google\Http\MediaFileUpload($this->client, $request, $mimeType, $data);
    ...
    public function createRequestUri($restPath, $params)
    {
        ...
        if (\count($uriTemplateVars)) {
            $uriTemplateParser = new \PostSMTPMailTester\Vendor\Google\Utils\UriTemplate();
