 ...
 */
namespace PostSMTPMailTester\Vendor\Google;

use PostSMTPMailTester\Vendor\Google\AccessToken\Revoke;
use PostSMTPMailTester\Vendor\Google\AccessToken\Verify;
use PostSMTPMailTester\Vendor\Google\Auth\ApplicationDefaultCredentials;
use PostSMTPMailTester\Vendor\Google\Auth\Cache\MemoryCacheItemPool;
...
use PostSMTPMailTester\Vendor\Google\Auth\OAuth2;
use PostSMTPMailTester\Vendor\Google\Auth\Credentials\ServiceAccountCredentials;
use PostSMTPMailTester\Vendor\Google\Auth\Credentials\UserRefreshCredentials;
use PostSMTPMailTester\Vendor\Google\AuthHandler\AuthHandlerFactory;
use PostSMTPMailTester\Vendor\Google\Http\REST;
use PostSMTPMailTester\Vendor\GuzzleHttp\Client as GuzzleClient;
use PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface;
use PostSMTPMailTester\Vendor\GuzzleHttp\Ring\Client\StreamHandler;
...
use PostSMTPMailTester\Vendor\Psr\Log\LoggerInterface;
use PostSMTPMailTester\Vendor\Monolog\Logger;
use PostSMTPMailTester\Vendor\Monolog\Handler\StreamHandler as MonologStreamHandler;
use PostSMTPMailTester\Vendor\Monolog\Handler\SyslogHandler as MonologSyslogHandler;
...
class Client
{
    ...
    public function __construct(array $config = array())
    {
        ...
        if (!\is_null($this->config['credentials'])) {
            if ($this->config['credentials'] instanceof \PostSMTPMailTester\Vendor\Google\Auth\CredentialsLoader) {
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \PostSMTPMailTester\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \PostSMTPMailTester\Vendor\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface $http = null)
    {
    ...
    {
        $tokenRevoker = new \PostSMTPMailTester\Vendor\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \PostSMTPMailTester\Vendor\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\PostSMTPMailTester\Vendor\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \PostSMTPMailTester\Vendor\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \PostSMTPMailTester\Vendor\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\PostSMTPMailTester\Vendor\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
    ...
    {
        $logger = new \PostSMTPMailTester\Vendor\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \PostSMTPMailTester\Vendor\Monolog\Handler\SyslogHandler('app', \LOG_USER, \PostSMTPMailTester\Vendor\Monolog\Logger::NOTICE);
        } else {
            $handler = new \PostSMTPMailTester\Vendor\Monolog\Handler\StreamHandler('php://stderr', \PostSMTPMailTester\Vendor\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \PostSMTPMailTester\Vendor\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param ClientInterface $http
     */
    public function setHttpClient(\PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        if (\defined('\\PostSMTPMailTester\\Vendor\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = \PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        } elseif (\defined('\\PostSMTPMailTester\\Vendor\\GuzzleHttp\\ClientInterface::VERSION')) {
            $guzzleVersion = (int) \substr(\PostSMTPMailTester\Vendor\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        if (5 === $guzzleVersion) {
            $options = ['base_url' => $this->config['base_path'], 'defaults' => ['exceptions' => \false]];
            if ($this->isAppEngine()) {
                // set StreamHandler on AppEngine by default
                $options['handler'] = new \PostSMTPMailTester\Vendor\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \PostSMTPMailTester\Vendor\GuzzleHttp\Client($options);
    }
    /**
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \PostSMTPMailTester\Vendor\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            ...
            $credentials = \PostSMTPMailTester\Vendor\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \PostSMTPMailTester\Vendor\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \PostSMTPMailTester\Vendor\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \PostSMTPMailTester\Vendor\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \PostSMTPMailTester\Vendor\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        ...
        return new \PostSMTPMailTester\Vendor\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
