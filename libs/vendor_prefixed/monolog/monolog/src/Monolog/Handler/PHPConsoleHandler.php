...
use Exception;
use PostSMTPMailTester\Vendor\Monolog\Formatter\LineFormatter;
use PostSMTPMailTester\Vendor\Monolog\Logger;
use PostSMTPMailTester\Vendor\Monolog\Utils;
use PostSMTPMailTester\Vendor\PhpConsole\Connector;
use PostSMTPMailTester\Vendor\PhpConsole\Handler;
use PostSMTPMailTester\Vendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \PostSMTPMailTester\Vendor\Monolog\Handler\AbstractProcessingHandler
{
    private $options = array(
        'enabled' => \true,
        // bool Is PHP Console server enabled
        'classesPartialsTraceIgnore' => array('PostSMTPMailTester\\Vendor\\Monolog\\'),
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \PostSMTPMailTester\Vendor\PhpConsole\Connector $connector = null, $level = \PostSMTPMailTester\Vendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('PostSMTPMailTester\\Vendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\PostSMTPMailTester\Vendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \PostSMTPMailTester\Vendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \PostSMTPMailTester\Vendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\PostSMTPMailTester\Vendor\PhpConsole\Helper::isRegistered()) {
            \PostSMTPMailTester\Vendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \PostSMTPMailTester\Vendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \PostSMTPMailTester\Vendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \PostSMTPMailTester\Vendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \PostSMTPMailTester\Vendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
