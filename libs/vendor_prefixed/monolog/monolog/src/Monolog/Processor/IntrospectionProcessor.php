 ...
 */
class IntrospectionProcessor implements \PostSMTPMailTester\Vendor\Monolog\Processor\ProcessorInterface
{
    private $level;
    private $skipClassesPartials;
    private $skipStackFramesCount;
    private $skipFunctions = array('call_user_func', 'call_user_func_array');
    public function __construct($level = \PostSMTPMailTester\Vendor\Monolog\Logger::DEBUG, array $skipClassesPartials = array(), $skipStackFramesCount = 0)
    {
        $this->level = \PostSMTPMailTester\Vendor\Monolog\Logger::toMonologLevel($level);
        $this->skipClassesPartials = \array_merge(array('PostSMTPMailTester\\Vendor\\Monolog\\'), $skipClassesPartials);
