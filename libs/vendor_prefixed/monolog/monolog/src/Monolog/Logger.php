...
use PostSMTPMailTester\Vendor\Monolog\Handler\HandlerInterface;
use PostSMTPMailTester\Vendor\Monolog\Handler\StreamHandler;
use PostSMTPMailTester\Vendor\Psr\Log\LoggerInterface;
use PostSMTPMailTester\Vendor\Psr\Log\InvalidArgumentException;
use Exception;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \PostSMTPMailTester\Vendor\Psr\Log\LoggerInterface, \PostSMTPMailTester\Vendor\Monolog\ResettableInterface
{
    /**
     ...
     * @return $this
     */
    public function pushHandler(\PostSMTPMailTester\Vendor\Monolog\Handler\HandlerInterface $handler)
    {
    ...
    {
        if (!$this->handlers) {
            $this->pushHandler(new \PostSMTPMailTester\Vendor\Monolog\Handler\StreamHandler('php://stderr', static::DEBUG));
        }
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \PostSMTPMailTester\Vendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \PostSMTPMailTester\Vendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName($level)
    {
        if (!isset(static::$levels[$level])) {
            throw new \PostSMTPMailTester\Vendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
