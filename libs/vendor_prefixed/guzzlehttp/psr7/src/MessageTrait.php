...
trait MessageTrait
{
    ...
    {
        if (!$this->stream) {
            $this->stream = \PostSMTPMailTester\Vendor\GuzzleHttp\Psr7\Utils::streamFor('');
        }
        return $this->stream;
    }
    public function withBody(\PostSMTPMailTester\Vendor\Psr\Http\Message\StreamInterface $body)
    {
        if ($body === $this->stream) {
            return $this;
        }
